...
> A point is a location in space. For our purposes, a location on a 2D plane, bound by **coordinates**.
...
Ruby has a rich standard library, of which includes mathematical functions. Needing the pythagorean theorem here, if we do some searching we can see Ruby already provides a method for us to calculate this instead of having to implement it ourselves:
...
```
> Here, the `x` in `x_difference = x - other.x` is actually calling this objects *own* `attr_reader` method internally that fetches its `@x` attribute. In another language, you may write something like `self.x` to refer to this value. You can do that here too in Ruby, but in most cases you can omit `self.`, as it is already understood what you mean.
...
When you're stuck, there's a lot of helpful ruby people out there. StackOverflow, IRC, etc. - or even me! I can be reached at wherever you're viewing this post, or at [my email](mailto:zachnowicki@gmail.com), or on [Discord](discordapp.com) - I'm `z64#2639`.
